...
import { pomodoroSessionModel, usersModel, workspacesModel } from "../../models";
...
const updatePomodoroSession = async (
 ...
 try {
   ...
   const session = await pomodoroSessionModel.findOne({
     _id: id,
     userId: user._id,
     workspaceId: workspace._id
   });
   ...
   session.startTime = startTime ? new Date(startTime) : new Date(session.startTime);
...
const deletePomodoroSession = async (
 ...
 try {
   ...
   const sessionDeleted = await pomodoroSessionModel.findOneAndDelete({
     _id: id,
     userId: user._id,
     workspaceId: workspace._id
   });
